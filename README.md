...
- **分层提示词**：人设层（app 级 `PERSONA_PROMPT` 可整体替换口吻，对所有入口生效）与系统指令层（输出格式如禁用 Markdown、记忆/工具、安全，可经 `SYSTEM_INSTRUCTIONS` 覆盖调试）分离，系统指令始终生效、优先于人设。
...
| `SYSTEM_INSTRUCTIONS` | 无 | 系统指令层覆盖（调试用），留空用内置默认；多行用字面量 `\n`。覆盖后需自含格式与安全约束 |
