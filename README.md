...
- **整池直供检索（零额外调用）**：不做向量匹配也不做关键词匹配，通常连「挑一挑」这一步都没有——活跃记忆不超过 `MEMORY_INLINE_MAX`（默认 300）时，候选池整个挂进主模型的 system 段，它一边看着全部记忆一边回话。零额外往返、零额外延迟，也不存在「挑漏了的记忆主模型再也看不到」这种查不出来的失败。记忆多到装不下时才降级成精选：交给同一个模型（关思考）挑出最相关的 `MEMORY_SEARCH_LIMIT` 条，**永不因检索失败中断对话**（见[数据结构](#数据结构)）。
- **核心记忆永远在场**：硬约束（过敏、忌口、健康与安全禁忌）与助手自己许下的承诺单独成段，**每轮无条件注入、永不参与精选**。分层只能保证它们进得了候选池，保证不了模型想得起来选它们——过敏这种事不该挂在一次软判断上。措辞上也与普通记忆区分：普通记忆是「背景印象、别主动提」，核心记忆是「可以不提，但绝不能违反」。
- **记忆演变（SUPERSEDES）**：用户情况变化时新记忆取代旧记忆并保留可回溯的时间线。
- **记忆主体（subject）**：区分“关于用户”与“助手自己的承诺 / 人设”，前者按普通记忆处理，后者随硬约束一起进核心段。
...
2. **核心层无条件注入**：硬约束（`kind = 'constraint'`）与助手自己的承诺（`subject = 'assistant'`）单独成段，**每轮直接进上下文，永不参与精选**。分层只能保证它们「进得了候选池」，保证不了模型「想得起来选它」——过敏这种事不该挂在一次软判断上。上限 `MEMORY_CORE_MAX`（默认 60），超了打 warn（多半是 `constraint` 标签被滥用了）。
3. **整池直供**：其余记忆不超过 `MEMORY_INLINE_MAX`（默认 300）时，整池原样挂进 system 段，模型一边看着全部记忆一边回话。**这就是全部**——一个模型、一次往返。
4. **降级精选**：只有其余记忆超过上限时，才把它编号后交给同一个模型（关思考）挑出相关的几条（最多 `MEMORY_SEARCH_LIMIT` 条），再映射回真实 id 取回正文。核心层不在这份清单里。

> `constraint` 到底算什么，在 `MEMORY_CONSOLIDATE_PROMPT` 与 `remember_memory` 的工具描述里有明确定义（「说错了会造成实际伤害或明确冒犯」，而不是「用户比较在意」）。分层依赖这个标签，所以那两处的措辞是判据的一部分，改判据就要一起改。
...
- **system 段按「稳定程度」排列**：人设 → 系统指令 → 核心记忆 → 其余记忆 → 会话摘要 → 当前时间 → 情绪 → 历史 → 本轮消息。越靠前越不变，因为任何一段变了都会把它后面的一切一起顶出缓存。核心记忆排在前面不只是因为它重要，更因为它**几乎不变**（只在写入新约束/新承诺时才动）——等于把稳定前缀又拉长一截。这在降级模式下尤其值钱：那时其余记忆每轮都在变，缓存本来只覆盖得到人设和系统指令。
