...
【记忆与工具】
- 系统会提供从私人记忆库检索出的内容；它们可能过期、矛盾或不相关，不能把它们当作用户本轮明确说过的话。但其中「硬约束与承诺」那一段是例外：它始终有效，你可以不主动提，却绝不能说出与之抵触的话。
...
// —— 核心记忆的开场白 ——
// 这一块每轮无条件注入，措辞要和下面的普通记忆区分开：普通记忆是「背景印象、别主动提」，
// 核心记忆是「可以不提，但绝不能违反」。两者搞混会让模型要么无视过敏，要么逢人就念叨它。
const CORE_BLOCK_HEADER: &str = "以下是关于这位用户的硬约束，以及你自己许下的承诺，\
每行开头方括号里是它的编号。它们始终有效，不因话题改变：\
不必主动提起，但你说的每一句都不能与之相抵触——尤其涉及饮食、健康、安全时，\
哪怕用户这次没提，也要按它们来。\
若用户明确说某条已经变了（不再过敏、约定作废），用那行的编号调 update_memory 更新它。";

// —— 普通记忆档案的开场白 ——
// 整池直供下模型会看到几百条记忆，「不要主动复述」这条约束比精选时代更吃重：
// 精选只给 8 条、大多真的相关，直供则必然混着大量与本轮无关的旧事。
const MEMORY_BLOCK_HEADER: &str = "以下是你对这位用户的其余长期记忆，按记住的先后排列，\
每行开头方括号里是它的编号。\
...
- kind：只能是 preference、fact、goal、relationship、constraint、event、other。
  其中 constraint 特指**必须始终顾及的硬约束**：过敏、忌口、健康或安全禁忌、药物冲突、用户明确要求「永远别做」的事。它会被永久置顶、每轮都进上下文并要求助手不得违反，所以判断标准是「说错了会造成实际伤害或明确冒犯」，不是「用户比较在意」。普通的喜好、习惯一律用 preference，别拿 constraint 装它们。
...
- 背景相关：虽没被直接提到，但会影响这次该怎么回答（如语言偏好、正在追的长期目标、最近反复出现的困扰）。注意硬约束与助手的承诺已经另行无条件注入，不在这份清单里，不用替它们操心。
...
fn builtin_tools() -> Vec<Value> {
    ...
    vec![
        ...
        json!({"type": "function", "function": {
            ...
            "parameters": {"type": "object", "properties": {
                "text": {"type": "string"},
                "kind": {"type": "string", "enum": kinds, "description": "constraint 特指必须始终顾及的硬约束（过敏、忌口、健康或安全禁忌、用户要求永远别做的事）——它会被永久置顶、每轮都进上下文，判断标准是「说错了会造成实际伤害或明确冒犯」；普通的喜好、习惯一律用 preference。"},
...
/// 一次上下文召回的结果。分成两段不是因为重要性不同，而是因为**稳定性**不同：
/// `core` 只在写入新的硬约束/承诺时才变（很罕见），`selected` 在降级模式下每轮都变。
/// 渲染时核心块排在前面，等于把稳定前缀又拉长一截，而不是往稳定区里插易变内容。
pub struct Recall {
    /// 核心记忆：硬约束与助手自己的承诺，永远在场、不参与精选。
    pub core: Vec<MemoryView>,
    /// 其余记忆：直供模式下是池子里剩下的全部，降级模式下是精选出的那几条。
    pub selected: Vec<MemoryView>,
}

impl Recall {
    /// 合并成一份平铺列表：回给 API 的 `retrieved_memories`、以及巩固器的「已有记忆」清单
    /// 都不关心这个分段，它们只想知道「这轮模型看见了哪些记忆」。
    fn into_flat(self) -> Vec<MemoryView> {
        let mut all = self.core;
        all.extend(self.selected);
        all
    }
}

/// 核心层判据：硬约束，以及助手自己对用户的承诺/设定。
///
/// 与 [`Store::memory_pool`] 里 `tier` 那段 SQL 是同一条规则的两份实现，必须同步修改。
/// `constraint` 的含义在 `MEMORY_CONSOLIDATE_PROMPT` 与 `remember_memory` 的工具描述里
/// 有明确定义——这个判据只有在标签贴得准时才成立，所以那两处的措辞是它的一部分。
fn is_core(candidate: &MemoryCandidate) -> bool {
    candidate.kind == "constraint" || candidate.subject == "assistant"
}

/// 核心层每轮都整段注入，是唯一会无节制侵占上下文的部分，所以要有个上限。
/// 撑到上限说明 `constraint` 标签被滥用了，值得一条 warn；保留最近写入的那些兜底。
fn cap_core(mut core: Vec<MemoryCandidate>) -> Vec<MemoryCandidate> {
    if core.len() <= config::MEMORY_CORE_MAX {
        return core;
    }
    tracing::warn!(
        "核心记忆 {}条 超过上限 {}，只注入最近写入的部分；\
         constraint 标签可能被滥用，建议 `mneme memory list` 核对",
        core.len(),
        config::MEMORY_CORE_MAX,
    );
    // 池子按 created_at 正序，尾部是最近写入的。
    core.drain(..core.len() - config::MEMORY_CORE_MAX);
    core
}
...
impl Agent {
    ...
    /// 上下文召回（对话热路径与记忆巩固都走这条）：核心层永远在场，其余装得下就**整池
    /// 直供**、装不下才降级精选。
    ///
    ...
    ///
    /// 记忆多到装不下时才退回精选，把上限压回 `MEMORY_SEARCH_LIMIT`——但**核心层不参与
    /// 精选**：分层只保证硬约束「进得了候选池」，不保证模型「想得起来选它」。过敏这种事
    /// 不能挂在一次软判断上，得由结构保证在场。
    pub async fn recall_for_context(&self, user_id: &str, query_text: &str) -> Result<Recall> {
        let pool = self.candidate_pool(user_id).await?;
        // 从池子里分核心层就够了，不必再查一次库：memory_pool 的 tier 排序把核心层排在
        // 最前面，LIMIT 永远先砍普通记忆。注意那条 SQL 里的 tier 判据与下面的 is_core
        // 必须保持一致，改一处就要改另一处。
        let (core_pool, rest_pool): (Vec<MemoryCandidate>, Vec<MemoryCandidate>) =
            pool.into_iter().partition(is_core);
        let core = self.take_all(user_id, cap_core(core_pool)).await?;

        if rest_pool.len() <= config::MEMORY_INLINE_MAX {
            tracing::debug!(
                "记忆整池直供 {}条（核心{}条，未调精选）",
                rest_pool.len(),
                core.len(),
            );
            let selected = self.take_all(user_id, rest_pool).await?;
            return Ok(Recall { core, selected });
        }
        // 这条 info 值得留：它标记着该用户已越过直供上限、开始为每轮多付一次模型往返。
        tracing::info!(
            "记忆池 {}条 超过直供上限 {}，降级精选（核心{}条仍直接注入）",
            rest_pool.len(),
            config::MEMORY_INLINE_MAX,
            core.len(),
        );
        let selected = self
            .degrade_to_select(user_id, query_text, rest_pool, config::MEMORY_SEARCH_LIMIT)
            .await?;
        Ok(Recall { core, selected })
    }
    ...
    pub async fn chat(
        ...
        // 都会把它后面的一切一起顶出缓存。人设/指令恒定 → 核心记忆几乎不变 → 其余记忆在
        // 直供下只在末尾追加（降级下每轮变）→ 摘要每次压缩重写 → 时间每分钟都在变 →
        // 情绪 → 历史 → 本轮消息。
        let core_block = format_core_block(&recall.core);
        let memory_block = format_memory_block(&recall.selected);
        ...
        if core_block.is_none() && memory_block.is_none() && summary_block.is_none() {
            messages.push(json!({"role": "system", "content": COLD_START_BACKGROUND}));
        }
        for block in [core_block, memory_block, summary_block]
            .into_iter()
            .flatten()
        {
            messages.push(json!({"role": "system", "content": block}));
        }
        let retrieved = recall.into_flat();
        messages.push(json!({
            "role": "system",
    ...
    async fn consolidate_batch(
        ...
        let existing = match self.recall_for_context(user_id, &retrieval_query).await {
            Ok(recall) => recall.into_flat(),
            Err(error) => {
                tracing::warn!("巩固前召回已有记忆失败，本批退化成纯新增：{error:#}");
                Vec::new()
            }
        };
...
/// 一条记忆渲染成一行：`[短编号] 正文`。
///
/// **一条一行**，不串成「A；B；C。」的散文——整池直供下这里可能有几百条，散文形态既不
/// 可读，也做不到「新增一条只在末尾多一行」这个前缀缓存要的形状。
fn render_lines(items: &[&MemoryView]) -> String {
    items
        .iter()
...
/// 核心记忆块：硬约束与助手承诺，**每轮无条件注入**，排在其余记忆之前。
///
/// 它比其余记忆更稳定（只在写入新约束/新承诺时才变，很罕见），所以放在前面是在把稳定
/// 前缀又拉长一截；降级模式下这一点尤其值钱——那时其余记忆每轮都在变，缓存本来只覆盖
/// 到人设和系统指令。
fn format_core_block(core: &[MemoryView]) -> Option<String> {
    if core.is_empty() {
        return None;
    }
    let (promises, constraints): (Vec<&MemoryView>, Vec<&MemoryView>) =
        core.iter().partition(|m| m.subject == "assistant");
    let mut parts: Vec<String> = vec![CORE_BLOCK_HEADER.to_string()];
    if !constraints.is_empty() {
        parts.push(format!("必须始终顾及的事：\n{}", render_lines(&constraints)));
    }
    if !promises.is_empty() {
        parts.push(format!(
            "你自己对用户的承诺或设定：\n{}",
            render_lines(&promises)
        ));
    }
    Some(parts.join("\n\n"))
}

/// 其余长期记忆：直供模式下是全部非核心记忆，降级模式下是精选出的那几条。
///
/// 这里不再按主体分组——`subject=assistant` 已经整体移进核心块了，剩下的都是关于用户的。
/// 直供模式下新记忆落在整段末尾，前面每一行都不动，前缀保持稳定。
fn format_memory_block(memories: &[MemoryView]) -> Option<String> {
    if memories.is_empty() {
        return None;
    }
    let items: Vec<&MemoryView> = memories.iter().collect();
    Some(format!(
        "{}\n\n关于用户，你还记得：\n{}",
        MEMORY_BLOCK_HEADER,
        render_lines(&items)
...
mod tests {
    ...
    #[test]
    fn memory_block_is_append_only() {
        // 前缀缓存的核心不变量：新增一条记忆，只应在整段末尾多一行，前面每个字节都不许动。
        let base = [
            view("22222222-bbbb", "用户偏好简洁的中文回答", "user"),
            view("33333333-cccc", "用户在 A 公司做后端", "user"),
        ];
        let before = format_memory_block(&base).unwrap();
        let after = format_memory_block(&[
            base[0].clone(),
            base[1].clone(),
            view("44444444-dddd", "用户养了一只叫年糕的猫", "user"),
        ])
        .unwrap();
        assert_eq!(
            after.strip_suffix("\n[44444444] 用户养了一只叫年糕的猫"),
            Some(before.as_str())
        );
        // 每行带 8 位短编号，模型可以直接拿它调 update/forget，省掉一次 search 往返。
        assert!(before.contains("[22222222] 用户偏好简洁的中文回答"));
        assert!(format_memory_block(&[]).is_none());
    }

    #[test]
    fn core_block_separates_constraints_from_promises() {
        // 硬约束与助手承诺同属核心层（每轮无条件注入），但分组呈现、互不混淆。
        let core = [
            view("aaaaaaaa-1111", "用户对花生过敏", "user"),
            view("bbbbbbbb-2222", "助手答应每天提醒用户喝水", "assistant"),
        ];
        let block = format_core_block(&core).unwrap();
        assert!(block.contains("[aaaaaaaa] 用户对花生过敏"));
        assert!(block.contains("[bbbbbbbb] 助手答应每天提醒用户喝水"));
        assert!(block.find("必须始终顾及").unwrap() < block.find("承诺或设定").unwrap());
        // 措辞必须和普通记忆块区分开：普通记忆是「别主动提」，核心记忆是「不得违反」。
        assert!(block.contains("不能与之相抵触"));
        assert!(format_core_block(&[]).is_none());
    }

    #[test]
    fn core_tier_covers_constraints_and_assistant_subject() {
        let candidate = |kind: &str, subject: &str| MemoryCandidate {
            id: "x".into(),
            text: "t".into(),
            kind: kind.into(),
            subject: subject.into(),
        };
        assert!(is_core(&candidate("constraint", "user")));
        assert!(is_core(&candidate("fact", "assistant")));
        assert!(!is_core(&candidate("preference", "user")));
        assert!(!is_core(&candidate("fact", "user")));
    }

    #[test]
    fn core_cap_keeps_the_most_recent() {
        // 核心层每轮整段注入，是唯一会无节制侵占上下文的部分；超限时保留最近写入的。
        // 池子按 created_at 正序，所以留的是尾部。
        let core: Vec<MemoryCandidate> = (0..config::MEMORY_CORE_MAX + 2)
            ...
            .collect();
        let capped = cap_core(core);
        assert_eq!(capped.len(), config::MEMORY_CORE_MAX);
        assert_eq!(capped[0].id, "id2");
        // 没超限时原样返回，不做任何裁剪。
        let small = vec![MemoryCandidate {
            id: "only".into(),
            text: "t".into(),
            kind: "constraint".into(),
            subject: "user".into(),
        }];
        assert_eq!(cap_core(small).len(), 1);
    }
